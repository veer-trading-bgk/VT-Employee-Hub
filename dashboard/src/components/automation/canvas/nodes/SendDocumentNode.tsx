...
export function SendDocumentNode({ data, selected }: NodeProps<CanvasNode>) {
  const cfg = data.config as SendDocumentConfig;
  const summary = cfg?.filename || cfg?.url || 'No file selected yet';
    ...
    <div className={cn(
      ...
      <div className="flex items-center gap-2.5">
        <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-neutral-100 dark:bg-neutral-800">
          <FileText className="h-3.5 w-3.5 text-neutral-500 dark:text-neutral-400" aria-hidden />
        </div>
        <div className="min-w-0">
          <p className="truncate text-sm font-semibold text-neutral-900 dark:text-white">Send Document</p>
          <p className="truncate text-xs text-neutral-500">{summary}</p>
        </div>
      </div>
      {cfg?.caption && (
        <p className="mt-1.5 truncate pl-9 text-[11px] text-neutral-400">&ldquo;{cfg.caption}&rdquo;</p>
      )}
