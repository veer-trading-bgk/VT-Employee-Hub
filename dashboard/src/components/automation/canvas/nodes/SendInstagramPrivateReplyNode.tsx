...
export function SendInstagramPrivateReplyNode({ data, selected }: NodeProps<CanvasNode>) {
  const cfg = data.config as SendInstagramPrivateReplyConfig;
  const variants = (cfg?.replyVariants ?? []).filter((v) => v.trim());
  const summary = variants.length > 1
    ? `${variants.length} reply variants (picked at random)`
    : (variants[0] || cfg?.messageText || 'No message set');
    ...
    <div className={cn(
      ...
      <div className="flex items-center gap-2.5">
        <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-neutral-100 dark:bg-neutral-800">
          <Reply className="h-3.5 w-3.5 text-neutral-500 dark:text-neutral-400" aria-hidden />
        </div>
        <div className="min-w-0">
          <p className="truncate text-sm font-semibold text-neutral-900 dark:text-white">Send Instagram Reply</p>
