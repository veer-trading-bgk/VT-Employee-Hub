...
export function CreateJourneyRecordNode({ data, selected }: NodeProps<CanvasNode>) {
  const cfg = data.config as CreateJourneyRecordConfig;
  const summary = cfg?.linkToLead ? 'Link to lead' : 'Record only';
    ...
    <div className={cn(
      ...
      <div className="flex items-center gap-2.5">
        <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-neutral-100 dark:bg-neutral-800">
          <ClipboardList className="h-3.5 w-3.5 text-neutral-500 dark:text-neutral-400" aria-hidden />
        </div>
        <div className="min-w-0">
          <p className="truncate text-sm font-semibold text-neutral-900 dark:text-white">Create Journey Record</p>
