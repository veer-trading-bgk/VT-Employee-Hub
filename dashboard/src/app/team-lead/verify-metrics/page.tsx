...
export default function TeamLeadVerifyMetricsPage() {
  const qc = useQueryClient();
  const [notes, setNotes] = useState<Record<string, string>>({});

  const { data, isLoading } = useQuery({
    queryKey: ['team-lead-pending-metrics'],
  ...
  const { mutate: verify, isPending: isVerifying } = useMutation({
    ...
    onSuccess: (_data, { approved }) => {
      ...
      qc.invalidateQueries({ queryKey: ['team-lead-pending-metrics'] });
    },
                ...
                <div className="flex flex-wrap items-start gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-3 text-sm text-slate-600 dark:text-slate-400">
                      <span className="font-medium uppercase text-cyan-600 dark:text-cyan-400">
                        {entry.metric_type}
                      </span>
                  ...
                  <div className="flex w-full flex-col gap-2 sm:w-52 sm:flex-shrink-0">
                    <textarea
                      ...
                      className="w-full rounded-lg border border-slate-200 px-3 py-1.5 text-xs focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 dark:border-slate-700 dark:bg-slate-800 dark:text-white"
                    />
