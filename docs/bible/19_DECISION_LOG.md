...
## Era 54 — Instagram DM automation v1: "lightweight, no CRM" second channel (2026-07-18)

**What shipped.** APForce's second messaging channel: Instagram DM keyword auto-reply. A new dedicated webhook route (`src/routes/instagram.js`), a new `InstagramSendService` (plain-text sends only), a new `InstagramContactService` owning `IGCONTACT#{companyId}#{igsid}` records, a new `InstagramTokenScheduler` refreshing 60-day tokens, and one new `send_instagram_message` automation node. Reuses `keyword_message` unmodified. See ADR-020.

**Preceded by a 6-point architecture audit, then a validation pass against two real reference implementations, then a locked scope correction — all before code (this same date).** The audit (Meta's live Instagram Messaging API docs, fetched and verified directly) found: the Instagram Messaging API is a genuinely separate webhook object (`object: "instagram"`, 16 fields vs WhatsApp's effectively one) requiring its own permission family (`instagram_business_basic`+`instagram_business_manage_messages`, the Instagram-Login family — no linked Facebook Page needed); the CRM-trigger layer (`tag_added`/`stage_changed`/`lead_created`/`stage_membership`/`keyword_message`) is already channel-agnostic by construction and needs zero engine changes; and the one real architectural blocker is identity — `CustomerIdentityService.resolveOrCreate()` hard-throws without a phone number (`CustomerIdentityService.js:616`, verified by direct read), and Instagram never provides one, only an opaque IGSID.

A validation pass against `github.com/fbsamples/graph-api-webhooks-samples` (archived; turned out to be a generic webhook-infrastructure skeleton, not Instagram-payload-specific — confirmed the X-Hub-Signature/hub.challenge handshake pattern and surfaced an alternative worth adopting: separate routes per platform rather than one shared route with an object check) and `github.com/ayuuxh2/insta-p8` (a real working Instagram-Login-family implementation) **confirmed** three load-bearing claims by direct code match (`story_mention` attachment-type check, `message.reply_to.story` field path, plain-text DM keyword matching) and **refined** two things the docs-only audit missed: comments arrive via a *separate* `entry.changes[]` array (WhatsApp-Cloud-API-shaped) while DMs/story-events arrive via `entry.messaging[]` (Messenger-Platform-shaped) — one webhook object, two envelope shapes — and Instagram Login tokens are short-lived-then-60-day-long-lived-refreshable, an operational concern the original audit's permission-scope focus hadn't surfaced.

**The scope-locking correction, mid-planning, before any code:** the first full implementation plan (built from the audit's recommended v1 boundary) proposed relaxing `CustomerIdentityService`'s phone requirement to "phone OR igsid," adding a `LEAD_IGSID#` lock, and treating Instagram contacts as `LEAD#` records. The business owner rejected this explicitly: **"Instagram is lightweight only, no CRM integration."** `CustomerIdentityService.js` is untouched — Instagram contacts are `IGCONTACT#{companyId}#{igsid}` records (`InstagramContactService.js`), never leads, no pipeline/stage/assignedTo, no `lead_created`/`tag_added`/`stage_changed` firing for Instagram at all. Simpler than CIS by construction: an IGSID has no normalization ambiguity, so no idempotency-lock/TransactWrite machinery is needed, just a conditional-put-if-absent.

**Tier reclassification, reasoned explicitly (not defaulted).** With CIS untouched, the standing CustomerIdentityService/WhatsAppSendService-specific Tier-1 rule no longer applies — but CLAUDE.md §11's own broader text ("any send/identity path"; "architecture decisions: new services") independently classifies this as Tier-1 regardless: a new send service, a new identity/contact-resolution path, a new webhook endpoint, a new scheduled job, and a new ADR. Treated as Tier-1 on that basis.

**Three validation-pass refinements incorporated into the final plan, each with a stated reason:** (1) a dedicated `/api/instagram/webhook` route, not a branch inside `whatsapp.js` — a wrong-shaped payload degrades to a visible no-op in a dedicated route vs. a silent one in a shared route with no prior shape check; (2) signature verification non-negotiable from line one, reusing `verifyMetaWebhookSignature()` completely unmodified — confirmed genuinely product-agnostic (validates against the app secret, not anything WhatsApp-specific) by direct code read; (3) `CONFIG#IG#{companyId}` carries a `tokenExpiresAt` field with a dedicated daily-throttled refresh sweep (`InstagramTokenScheduler.js`, riding the existing 5-minute EventBridge tick exactly like `LeadScoringScheduler.js`'s cursor pattern, zero new AWS provisioning) — a requirement with no WhatsApp analog.

**A self-caught correctness bug during test-writing, fixed before self-test:** `InstagramTokenScheduler`'s sweep loop originally counted `_refreshOne()`'s non-throwing "config vanished between scan and refresh" outcome (`{skipped:true}`) as a successful refresh — inflating `refreshedCount` for a company that was actually skipped. Fixed to track `skippedCount` separately; caught while writing `instagramTokenScheduler.test.js`'s bounded-scan-pagination test, before it ever reached a real sweep.

**A second self-caught test-design bug:** `InstagramSendService` initially read Instagram credentials via `getCachedIgConfig()` (a 10-minute in-process cache) — mirroring `WhatsAppSendService`'s choice. But `WhatsAppSendService`'s cache exists specifically to avoid N reads in a broadcast loop, a shape Instagram sends don't have; worse, the cache Map persists across tests in the same file, so a test's `mockConfig()` call would silently no-op for any companyId already cached by an earlier test. Fixed to use the always-fresh `getIgConfig()`, matching `CapiService`/`FlowManagementService`'s already-established choice for non-broadcast-loop services — architecturally more correct, not just a test workaround.

**`lucide-react` doesn't export an `Instagram` icon** (brand/social icons were removed from the library) — surfaced by `tsc`, not caught during writing. Substituted `Camera` across all four UI surfaces (palette, config panel, canvas node, Settings section) rather than adding a custom SVG for a v1 feature.

**Purely additive, confirmed:** dispatch gains one `send_instagram_message` case; `keyword_message`'s matcher, trigger dispatch, and every existing trigger type are untouched; `app.js` gains one new route mount with no blanket auth wrapper (matching `whatsapp.js`'s exact shape, since the webhook GET/POST must stay public); `handler.js`'s `Promise.allSettled([...])` gains one more self-throttled sweep call, same pattern as three existing ones.

**Tests (48 new).** `tests/instagramContactService.test.js` (create/get-existing/username-refresh/concurrent-create-race, explicit negative assertions that no lead-shaped field — `stage`/`assignedTo`/`pipelineId`/`phone` — ever appears on an `IGCONTACT#` item, best-effort message-write failure swallowed not thrown). `tests/instagramSendService.test.js` (config-gate rejection matrix, payload contract — `recipient.id`/`message.text`/Bearer auth — multi-tenant non-cross-contamination). `tests/instagramWebhook.test.js` (GET handshake, signature-gate wiring proven via a controllable mock rather than re-testing HMAC math the signature utility already owns, `entry.changes` stub, plain-DM keyword fire with `ctx.igsid` populated, story-reply/mention stubs skip cleanly, no-company-mapped skip, always-200-even-on-internal-error). `tests/instagramTokenScheduler.test.js` (self-throttle cursor, refresh-window boundary, per-company failure isolation, bounded-scan pagination — this is the test that caught the skippedCount bug above). `tests/automationEngine.test.js` (+4: `ctx.igsid` read directly not `leadPK`, missing-igsid/missing-messageText rejection, failure propagation). Full backend suite 2020/2020 green; dashboard eslint + `tsc --noEmit` clean after three real `tsc`-caught fixes (missing `Instagram` icon export, `SettingsSection`/`BadgeVariant` union gaps from a Settings-page precedent I misread as already using `'neutral'` when it was actually my own just-inserted code).

**Open items, explicitly NOT closed by this change:** (1) the `/oauth/authorize` dialog URL/params are the standard documented shape for the Instagram Login family, not independently doc-fetched this session (the token-exchange URLs were, against `insta-p8`'s real code) — smoke-test against the live Meta App Dashboard config before production; (2) v1 is genuinely headless — no dashboard surface reads `IGCONTACT#` conversation history; a "view Instagram conversations" screen is an explicit future decision, not scoped; (3) comment-to-DM, story reply, story mention, and live comment automation are all structurally stubbed (logged, 200'd) in the webhook route, ready for v2 without a route redesign, but not built; (4) the 24-hour-standard-messaging-window escape-hatch question flagged in the original audit (whether Instagram has any WhatsApp-template-style outside-window send mechanism) remains unverified — irrelevant to v1's synchronous keyword-reply shape, but relevant before any future multi-step/delayed Instagram flow.

**Reference:** `src/routes/instagram.js`, `src/services/InstagramSendService.js`, `src/services/InstagramContactService.js`, `src/services/InstagramTokenScheduler.js`, `src/services/igGraphApiHelpers.js`, `src/core/entityKeys.js` (`igConfigPK`/`igIdConfigPK`/`igContactPK`), `src/services/AutomationEngine.js` (`send_instagram_message` case), `src/handler.js` (scheduler wiring), `dashboard/src/types/automations.ts` (`SendInstagramMessageConfig`), `dashboard/src/components/automation/SendInstagramMessageEditor.tsx` + `canvas/nodes/SendInstagramMessageNode.tsx` + palette/panel/canvas registrations, `dashboard/src/app/(v3)/settings/page.tsx` (`InstagramSection`), `docs/adr/ADR-020-instagram-send-service-boundary.md`.

---

## Era 55 — Instagram Comment-to-DM v2: private replies + the Follow Gate (2026-07-19)

**What shipped.** The `entry.changes[]` (comments) branch ADR-020 left as a structural stub is now real: a comment on a targeted post/Reel fires a new `comment_received` trigger, sends a private-reply DM (DM #1), and — for the Follow Gate two-step — waits for the user's reply and sends DM #2. One backend PR, headless (no canvas UI yet), predominantly Tier-1 (new send path + resume/identity mechanism + signature-gated webhook logic). See ADR-021.

**Preceded by an 8-question scoping pass with two Meta-doc verifications, before any code.** `POST /{ig}/messages` with `recipient: { comment_id }` is the private-reply shape (verified against Meta's Private Replies doc), returning `recipient_id` — the commenter's canonical IGSID. The comment webhook `value` carries `id`/`text`/`from.id`/`media.id`/`parent_id` (verified against Meta's Webhooks reference). Three Meta constraints drove the design, not preference: exactly ONE private reply per comment; within 7 days; and "follow-ups only if the recipient responds, within 24h" — which *forces* DM #1 = private reply (comment_id) and DM #2 = normal DM (igsid), the exact Follow Gate shape.

**The honest correction to the original v1 audit.** The audit said multi-step DM flows need "no new engine mechanism." True of state machinery — the `AUTO_WAIT#` storage, `_storeWait`, `resumeExecution`, `_claimAndResume`, the `processAllDueWaits` time-sweep, and the graph pause/resume mechanics are all reused unchanged. But the only event-driven resume trigger, `resumeOnButtonReply`, is WhatsApp-specific: it keys on `to10Digit(awaitReply.phone)` and requires a button tap in `expectedButtonIds`. Instagram DM replies are free text, keyed by IGSID. So the Follow Gate needed one genuinely new (small, pattern-parallel) primitive — `resumeOnInstagramReply(companyId, igsid)` plus a `wait_instagram_reply` pause node storing `awaitReply: { igsid }`. Reported to the business owner as the headline finding before implementation; not glossed.

**Isolation, verified.** IG waits store no `phone`; WhatsApp waits store no `igsid`. `resumeOnButtonReply`/`cancelButtonReplyWaits` filter on `awaitReply.phone` (and `to10Digit(undefined) === ''` never equals a real 10-digit), and `resumeOnInstagramReply` filters on `awaitReply.igsid` — so neither resume path can ever cross-fire into the other channel's paused executions. Covered by an explicit test (a button wait + a different-igsid wait both correctly ignored).

**Guards the one-shot private reply forced.** (1) Per-comment idempotency: a `IGCOMMENT#{companyId}#{commentId}`/`CLAIM` marker (`dedupPut`, 30-day TTL) written before the automation fires, so a Meta webhook retry is a no-op, not a second failing send. (2) Self-comment guard: `value.from.id === entry.id` (the business account commenting on its own post) is skipped — the comment-side analog of the DM `is_echo` guard shipped the same week (commit 92a9ec3). (3) Top-level only: a `parent_id`-bearing reply-to-comment is skipped. (4) A resuming DM is *consumed* by the gate and does NOT also fire `keyword_message` (locked decision, mirrors `cancelButtonReplyWaits`).

**Reuse, confirmed.** `comment_received`'s config reuses `keyword_message`'s exact shape (`keywords[]`/`matchMode`/`caseSensitive`) so the keyword matcher is shared verbatim — refactored to a neutral `_keywordHit` used by both `_matchesKeywordConfig` and the new `_matchesCommentConfig` (which adds a required `mediaId` string-equality check, fail-closed on blank). The private reply goes through `InstagramSendService.sendPrivateReply` (ADR-021 R1, extends ADR-020 R1); contacts stay `IGCONTACT#`, keyed on the response `recipient_id` (ADR-020 R2). Anti-spam `replyVariants: string[]` (≥2, random pick) is a small helper on both send nodes, not a new node.

**Webhook dispatch made defensive, not assumption-based (review catch).** The first cut had a comments-priority early return (`if (hasComments) { ...; return 200 }`) and read only `entry[0]`, on the assumption that a delivery carries one field type. Asked to verify that against Meta's real docs before approval, the check found the opposite: Meta does NOT document that a single entry's `changes[]` (comments) and `messaging[]` (DMs) are mutually exclusive, and it explicitly warns batching "cannot be guaranteed... handle each Webhook individually" — batching being at the `entry[]` level. So the dispatch now iterates EVERY entry, resolves company per entry (a batch can in principle span accounts), and processes both branches independently within each, each entry isolated in its own try so one failure never drops its siblings. Two tests pin it: an entry carrying both a comment and a DM fires both `comment_received` and `keyword_message`; two batched entries both process. The earlier "always ACKs 200 on internal throw" guarantee is preserved (per-entry catch uses the same log message).

**Also shipped:** a read-only `GET /api/instagram/media` (admin) as the future mediaId picker's data source — no picker UI in this backend-only pass.

**Tests (~30 new; full suite green).** `instagramSendService.test.js` (+sendPrivateReply: comment_id payload contract, recipient_id capture, records against the response IGSID not the comment_id, Meta-error mapping). `instagramWebhook.test.js` (+comment fire with claim, self-comment guard, parent_id skip, idempotency dup no-op, non-comments field skip, resume-consumes-keyword; the obsolete v1-stub test replaced by a malformed-comment no-op). `automationEngine.test.js` (+`_matchesCommentConfig`, `_pickInstagramVariant`, `send_instagram_private_reply` node, `send_instagram_message` variant support, the full Follow Gate: DM #1 → `wait_instagram_reply` pause keyed on the response IGSID → `resumeOnInstagramReply` sends DM #2 → cross-channel isolation → timeout ends without DM #2). `automationsTriggerStorage.test.js` (+`comment_received` validation/storage: sanitized keywords + trimmed mediaId, mediaId-required 400, empty-keywords 400, bad-matchMode 400).

**Open items, explicitly NOT closed:** (1) canvas UI — trigger config (mediaId picker over `GET /media`), the new nodes, and the variant editor — is a separate frontend PR; until then `comment_received` workflows are configured by direct DynamoDB writes. (2) "All posts" targeting deferred to v3. (3) Live-broadcast comment replies (broadcast-window-only) out of scope. (4) Public comment reply (vs private DM) not built. (5) `send_instagram_private_reply` as the required entry node for a comment workflow is not enforced (headless) — a canvas UI should guide it.

**Reference:** `src/routes/instagram.js` (`processCommentEvent`, `GET /media`, resume wiring), `src/services/InstagramSendService.js` (`sendPrivateReply`), `src/services/AutomationEngine.js` (`_keywordHit`/`_matchesCommentConfig`, `_pickInstagramVariant`, `send_instagram_private_reply`, `wait_instagram_reply`, `resumeOnInstagramReply`), `src/routes/automations.js` (`validateCommentTriggerConfig`), `src/core/entityKeys.js` (`igCommentClaimPK`/`igCommentClaimSK`), `docs/adr/ADR-021-instagram-comment-to-dm.md`.

---

## Era 56 — Instagram page (v3): consolidated Messages/Comments/Settings workspace, 4 PRs (2026-07-19)

**What shipped, as one completed unit.** A dedicated `/instagram` page consolidating DM conversations, post-grouped comments, and Instagram connect/disconnect settings into a single admin-only workspace — reached from the Inbox page via a new WhatsApp/Instagram icon split next to the (untouched) Broadcast button. Read-only visibility plus existing settings; the locked "no `LEAD#`, no CRM" stance from Era 54/55 is unchanged throughout. Governed by ADR-022 (comment storage shape + interim-Scan-vs-GSI decision), sequenced as 4 backend-first PRs:

- **PR1 — data model** (`333d68c`): a new `IGPOST#{companyId}#{mediaId}` entity (`InstagramCommentService`) storing comments as readable, post-grouped records going forward (`CMT#` items + a `META` summary with best-effort `totalComments`/`unrepliedComments` counts) — closing the gap where comments were never persisted, only DMs were. `processCommentEvent` now requires `mediaId`; the private-reply node flips a comment's `replyStatus` on send via a conditional transition (decrements the count exactly once).
- **PR2 — backend reads + real-time** (`8ae8f82`, CI run #545 green): four admin-only routes — `GET /contacts` (interim Scan per ADR-022, + a `pendingFollowGate` flag from a new `AutomationEngine.pendingInstagramReplyIgsids()`, which reuses the exact `AUTO_WAIT#` query/filter `resumeOnInstagramReply` already used, so the two can never disagree), `GET /contacts/:igsid/messages`, `GET /posts`, `GET /posts/:mediaId/comments`. Real-time: `notifyCompany('instagram_message')` / `('instagram_comment')`, wired into the dashboard's `EVENT_QUERY_MAP` (→ `['instagram-contacts']` / `['instagram-posts']`).
- **PR3 — frontend page + manual reply** (`b9d1b28`, CI run #546 green including the Vercel build): the two-tab `/instagram` page. Messages is a FORK of the WhatsApp Inbox's `ConversationList`/`ThreadPane` (not reuse — those are inline in the 2,500-line `inbox/page.tsx`, hard-bound to `WaConversation`/`WaMessage`), stripped to igsid-keyed/text-only/no-CRM. Comments is post-grouped (posts list with an unreplied badge → per-post comments with an inline private-reply composer), hitting a new `POST /posts/:mediaId/comments/:commentId/reply` (admin-only send path — refuses an already-`replied` comment with 409, Meta's one-private-reply-per-comment limit, else the same `sendPrivateReply` + `markCommentReplied` path the automated Follow Gate uses). Settings was a temporary, deliberately self-contained standalone panel (PR3 touched no existing files).
- **PR4 — nav split, Settings consolidation, docs (this change).** The Inbox page's `[Inbox][Broadcast]` mode switcher becomes `[WhatsApp icon][Instagram icon][Broadcast]`: WhatsApp is the existing `inbox` mode, byte-identical behavior, now icon-labeled; Instagram is a genuine `<Link>` navigation to `/instagram` (no corresponding `CommPageMode` value — it was never a "mode"); Broadcast is untouched. Two hand-authored, lucide-styled brand glyphs (`components/icons/BrandIcons.tsx`) replace the generic `Camera` placeholder — not pixel-exact official logos, explicitly commented as such. PR3's temporary standalone Settings panel was extracted into the definitive shared `InstagramSettingsPanel`, and the main Settings page's Instagram section (its `SectionDef` entry, its `SettingsSection` union member, and its `renderContent()` case — all three touch-points) was **removed entirely**, not left pointing at the shared component: per the feature's original design ("consolidates... replacing the standalone Settings → Instagram panel, not duplicating it"), Instagram is now configured from exactly one place.

**Two real findings caught during PR4's mandated end-to-end trace, both fixed in this same change (not left as follow-up items):**
1. **Stale header badges.** `MessagesTab`/`CommentsTab` each stored the *selected* contact/post as its own React state snapshot at click-time. A Follow Gate resolving (or an unreplied count changing) while that thread/post was open wouldn't update the open header until the user re-selected it — even though the underlying list correctly refetched on every WS event, since the snapshot was never resynced against the fresher query result. Fixed by lifting each tab's `useQuery` into the parent component and deriving the "active" object fresh every render via `.find(id)` against the live list, rather than storing a copy. This is a genuine correctness fix to PR3's own code (not pre-existing debt), found by literally tracing the WS → cache-invalidation → render path end to end rather than assuming PR1–3 composed correctly untested.
2. **Non-admin click-then-bounce.** Unlike WhatsApp/Broadcast, `/inbox` itself has no role gate — any authenticated user reaches the switcher — but the Instagram destination is admin-only end to end (every route `checkRole(['admin'])`, the page `ProtectedRoute allowedRoles={['admin']}`). Without a check, a non-admin would see the icon, click it, and get redirected away. Fixed: the Instagram icon now only renders for `admin`/`superadmin`, mirroring the same admin-gating the Settings page's old `adminOnly: true` entry already enforced (superadmin bypasses, exactly like `checkRole()`/`ProtectedRoute` do everywhere else).

**A pre-existing, unrelated issue surfaced but deliberately NOT touched:** `eslint` on `inbox/page.tsx` reports one `react-hooks/purity` error (`useState(Date.now())` at line ~1170) and three warnings (`<img>` LCP, `aria-selected` on a `button` role) — confirmed via `git diff`'s hunk boundaries and a `git show HEAD` byte-comparison to be completely untouched by every PR4 edit (existed identically before PR4 and evidently doesn't block Vercel's actual build, since PR3's CI run #546 — touching this same file with this same pre-existing issue — deployed successfully). Left as-is: fixing it would be scope creep for a nav/consolidation/docs change.

**Tests.** Backend: zero new backend files in PR4 (nav/Settings/docs only) — full suite reconfirmed green (2088/2088, unchanged from PR3). Frontend: `tsc --noEmit` clean and `eslint` clean on every PR4-touched file (the one pre-existing, unrelated finding above aside); the dashboard has no component-render test tooling (Playwright E2E only, an existing, accepted gap — see PR3's own note), so CI's Vercel `next build` remains the frontend's real integration gate.

**Open items, explicitly NOT closed by this change:** (1) the Messages/Comments tabs' list views stay Scan-based per ADR-022 D2.3's documented graduation trigger (revisit at ~500–1,000 contacts/comments per company, or ~20–30 IG-active companies) — not yet reached, not built preemptively. (2) "All posts" comment targeting remains v3-deferred (ADR-021). (3) No canvas UI for `comment_received`/Follow-Gate workflow authoring — still configured by direct DynamoDB writes. (4) Live-broadcast comment replies and public (non-private) comment replies remain out of scope.

**Reference:** `dashboard/src/app/(v3)/instagram/page.tsx`, `dashboard/src/app/(v3)/inbox/page.tsx` (channel switcher), `dashboard/src/app/(v3)/settings/page.tsx` (Instagram section removed), `dashboard/src/components/instagram/` (`MessagesTab.tsx`, `CommentsTab.tsx`, `InstagramSettingsPanel.tsx`, `types.ts`), `dashboard/src/components/icons/BrandIcons.tsx`, `dashboard/src/contexts/WebSocketContext.tsx` (`EVENT_QUERY_MAP`), `src/routes/instagram.js` (4 read routes + manual reply route), `src/services/AutomationEngine.js` (`pendingInstagramReplyIgsids`), `docs/adr/ADR-022-instagram-page-comment-storage-and-listing.md`.
