...
| **Automations** | Shipped and, as of 2026-07-04, branching — `AutomationEngine.js` runs either a legacy flat `steps[]` pipeline or a new graph (`nodes[]`/`edges[]`) with if/else `condition` nodes (CRM-field match with live re-fetch, or WhatsApp button-reply branching), sharing one distributed-claim wait/resume infra across both shapes so concurrent ticks can't double-resume a paused execution. Built after ADR-012/013 existed and correctly cites both in its own commit message and code comments. `tests/automationEngine.test.js` has 14 tests (this was already non-zero before 2026-07-04 — an earlier version of this line claiming "zero automated test coverage" was itself stale, not something this update caused); route-level `automations.js` and `whatsapp.js` still have no dedicated tests. The React Flow visual canvas for building graph workflows is not yet built (backend-only so far). *(08_MODULES.md `AutomationEngine.js`; 19_DECISION_LOG.md Era 6, Era 10)* |
