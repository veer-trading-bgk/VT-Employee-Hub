...
**Update 2026-07-12:** §2 and §4 below received targeted updates (Track A closure, Templates/OAuth
fixes, Track B2 progress, Meta App Review status) per `19_DECISION_LOG.md` Era 44 — this was a
scoped patch of specific, named items, not a full chapter regeneration; §3's critical findings and
§5's open policy decisions were not re-audited in this pass and may themselves have drifted further
since 2026-07-02.
**Update 2026-07-17:** scoped patch — the in-app WhatsApp Flow builder (§2 WhatsApp messaging row)
shipped; nothing else re-audited.
**Update 2026-07-29:** scoped patch — Meta Embedded Signup (§2 WhatsApp messaging row, Era 57, ADR-024)
shipped across 3 PRs but is not yet live-usable (blocked on a Meta-console `config_id` only the user can
create); nothing else re-audited.
...
|---|---|
| **WhatsApp messaging** | Fully shipped and centralized. Every real outbound send (text/template/interactive/media) goes through `WhatsAppSendService` per ADR-012, with real-time delivery via a WebSocket layer and S3-backed media handling that keeps large payloads out of the Lambda request path. One narrow, known bypass exists (read-receipts) — see §3. **Fixed 2026-07-10:** template media headers (IMAGE/VIDEO/DOCUMENT) now work at both template-creation time (real Resumable Upload API integration — no template, for any company, had ever had a working media header before this) and send time (a separate, previously-missing header-parameter build for approved media-header templates); and the OAuth "Connect with Meta" flow, which was rejecting 100% of attempts over one invalid requested scope (`business_management`), now connects successfully. **2026-07-17:** the in-app WhatsApp Flow builder is shipped (`FUTURE_EXTENSIONS.md` §13 delivered): backend `FlowManagementService` wraps Meta's Flow Management API (create/upload/publish/preview — a sibling to `WhatsAppSendService`, sends stay ADR-012-governed) behind admin-only `whatsapp.js` `/flows/builder` routes, and the frontend multi-screen editor (`dashboard/src/types/flowBuilder.ts` serialization with cross-screen `data`-schema passing + `dashboard/src/components/flow-builder/`) is reachable via Settings → WhatsApp Flows → "Build a Flow" (`/settings/flows/builder`); registering an externally-built Flow by ID remains supported unchanged. **2026-07-29 (Era 57, ADR-024, 3 PRs):** Meta's real Embedded Signup shipped as a third `setupMethod` (`embedded_signup`) alongside `manual`/`oauth` — `EmbeddedSignupService.js`'s 6-step automatic configuration pipeline, a Drawer-based wizard, a duplicate-phone-number guard, and per-`setupMethod` token encryption (only `embedded_signup` tokens are encrypted at rest; existing `manual`/`oauth` tokens stay plaintext, deliberately un-migrated). **Not yet live-usable**: blocked on the user creating a Meta App Dashboard `config_id` and setting `META_EMBEDDED_SIGNUP_CONFIG_ID` — until then the feature's entry point stays disabled and the existing connect methods are the working fallback. *(06_ARCHITECTURE.md §4a-4b, §7; 08_MODULES.md `WhatsAppSendService`; 19_DECISION_LOG.md Era 44, Era 57; ADR-024)* |
...
| **CRM / Customer 360** | The CRM pipeline (lead stages, assignment, follow-ups, CSV import) and the Customer 360 detail page (7 frozen tabs: Profile, Conversation, Timeline, CRM, Tasks, Notes, Documents) are both shipped and in daily use. The identity layer underneath them (which service is authoritative for creating a customer) is **not** actually wired up — this is the single biggest finding in this document, see §3. As of 2026-07-12: "Track A," a 9-item Contacts/Inbox fix pass (CSV import truncating around ~30 contacts, Kanban/List truncating at 100 leads, Inbox notes visibility/save + unread tab + panel width, among others), is confirmed closed — shipped across 2026-07-08 to 2026-07-10. One real gap surfaced *during* Track A and deliberately left open, not fixed: `contacts.js`'s `team_lead` RBAC scope is own-assigned-only, while `09_PERMISSION_MATRIX.md` documents team-wide visibility — a genuine pending product decision, not a bug fix, tracked in `docs/PENDING_WORK.md`. *(08_MODULES.md `crm.js`, `contacts.js`; 19_DECISION_LOG.md Era 3, Era 44)* |
...
| **Automations** | Shipped and, as of 2026-07-04, branching — `AutomationEngine.js` runs either a legacy flat `steps[]` pipeline or a new graph (`nodes[]`/`edges[]`) with if/else `condition` nodes (CRM-field match with live re-fetch, or WhatsApp button-reply branching), sharing one distributed-claim wait/resume infra across both shapes so concurrent ticks can't double-resume a paused execution. Built after ADR-012/013 existed and correctly cites both in its own commit message and code comments. As of 2026-07-05, a `keyword_message` trigger type also exists — fires on an inbound WhatsApp text message or button/list tap matching an author-configured exact/contains/any-of-list keyword rule, editable from both the linear drawer's trigger dropdown and (new) the branching canvas's own trigger panel; this same change fixed a real, previously-silent gap where a tapped Message+List row was dropped by the webhook entirely (not stored, no WS push — the node type had no working inbound counterpart until now). `tests/automationEngine.test.js` has 58 tests as of this commit (verified by direct run — treat the file's own historical "14 tests" figure as already stale before this change, not something this update caused); `tests/automationsRoutes.test.js` and a new `tests/whatsappListReply.test.js` add route- and webhook-level coverage for this trigger specifically (this line previously claimed route-level `automations.js`/`whatsapp.js` had no dedicated tests at all — narrow that to "no *full-handler* tests for whatsapp.js's webhook existed before 2026-07-05"; the established, still-current convention for that file is unit-testing its exported pure helpers, e.g. `isButtonReply`/`sendWelcomeMessage`, not full HTTP-level tests). Also as of 2026-07-05: `send_buttons`/`send_list` graph nodes can pause themselves directly via per-button/row canvas handles (opt-in — only when a handle-specific edge exists; every pre-existing single-default-edge workflow is unaffected), and a new `inbound_webhook` trigger type lets an external system POST to a capability URL (`/api/automations/webhook/:companyId/:workflowId/:token`, public/no-auth, token-gated via `crypto.timingSafeEqual`) to run one named workflow directly via the new `AutomationEngine.runWorkflowDirect()`, resolving the contact through `CustomerIdentityService.resolveOrCreate()` (ADR-013) same as every other lead-creating entry point. **Fixed 2026-07-06 (production incident, first real customer use of the button-tap pause/resume feature):** `_finalizeExecution()` was interpolating the reserved DynamoDB keyword `path` raw into an `UpdateExpression`, silently crashing (caught + logged, not surfaced) every single graph execution's finalize since the graph engine shipped — 100% of real resumes failed, instantly, not "delayed." Also fixed: `processDueWaits()`'s documented EventBridge wiring never actually existed — added `processAllDueWaits()`, now wired to the existing 5-minute tick alongside `runDueCampaigns()`/`runDueLeadScoring()`, so paused-workflow timeouts finally have a working fallback sweep. All 19 stuck/anomalous test executions on the affected workflow were data-deleted (pre-launch, no real customers yet) and verified gone. A codebase-wide sweep found one other (currently-safe, now hardened defensively) site with the same raw-dynamic-attribute-name shape — `whatsapp.js`'s broadcast/campaign stats `ADD` clauses — everything else already properly `#`-aliases dynamic field names. **Fixed 2026-07-06 (Era 20):** `whatsapp.js`'s `resumeOnButtonReply()` (×2), `fireTrigger('keyword_message', …)` (×2), and `runAutomations('whatsapp_conversation_started', …)` (×1) were all fire-and-forget before the webhook's own `res.sendStatus(200)` — measured 6.3-49.4s delays (mean ~22s) and 2 executions that never completed at all. `fireTrigger()`'s own per-workflow dispatch loop was *itself* fire-and-forget too, so the real fix lives in `AutomationEngine.fireTrigger()` (now `await Promise.allSettled(...)`, benefiting `crm.js`/`forms.js`/`campaigns.js` as well), plus all 5 `whatsapp.js` call sites now awaited with a bounded 5s timeout race (`withTimeout()` — chosen against the real, verified 30000ms ceiling shared by Lambda's configured `Timeout` and API Gateway's HTTP API integration `TimeoutInMillis`, since neither Meta's Graph API Webhooks docs nor the WhatsApp Cloud API webhook-setup docs publish a response-time SLA). `crm.js`'s 3 call sites are deliberately left as-is — same risk, different blast radius, flagged for a separate future decision. **2026-07-10:** the linear "Simple" editor is removed — the branching canvas is now the sole workflow editor (the one real production linear workflow was losslessly converted, not left running dual-shape code). **2026-07-12, Track B2:** Batch 1 (canvas Save-panel visibility fix, delete-node affordance, categorized node palette, dashboard Success Rate `GaugeChart`, Welcome/Working-Hours/Delayed-Response settings relocated out of the Workflows tab into a new Settings tab) and Batch 2a (per-workflow success/failure health counters, real `GET /executions` pagination + server-side search/sort, shared `Table`/`SearchBar`/`FilterBar` component adoption) are both shipped. **Still queued, not started:** item 9 — execution-volume/trigger-breakdown charts — needs its own aggregation-strategy scoping pass; see `docs/PENDING_WORK.md`. **2026-07-17:** a `flow_completed` trigger type is wired — fires from the webhook's `nfm_reply` path when flowId correlation resolves (including ambiguous best-guess matches; never for uncorrelated `flowId: null` responses), filterable per-workflow to one specific Flow via `trigger.config.flowId` (blank = any-Flow catch-all; fails open on missing config, the deliberate opposite of `keyword_message`'s fail-closed), with a `FlowPicker` config editor in the shared TriggerEditor; its three prerequisites (send_flow node, flowId correlation, `FlowResponsesByCompany` GSI) all shipped 2026-07-16/17. **2026-07-18:** a "Meta Signal" (`meta_signal`) action node reports CRM conversions to Meta's Conversions API for Business Messaging via a new `CapiService` (ADR-019, sibling to `WhatsAppSendService` — sends stay ADR-012-governed): dropdown of Meta's fixed 14-event list (custom names unsupported on this channel, doc-verified), once-ever-per-lead-per-event via a no-TTL claim-first `CAPI#` marker (Meta does NOT dedup BM events), lazy dataset auto-provisioning cached as `capiDatasetId` on `CONFIG#WABA#`, optional `expectedValue`-sourced `custom_data.value` (INR), and a TTL'd `CAPILOG#` observability log (`sent|failed|skipped`+reason; no admin surface yet). Operational prerequisite, not code: the token needs `whatsapp_business_manage_events` advanced access, applied for in the Meta App Dashboard. **2026-07-18:** Instagram DM automation v1 — APForce's second messaging channel — shipped: a dedicated `/api/instagram/webhook` route (`InstagramSendService`, `InstagramContactService`, `InstagramTokenScheduler`, ADR-020), reusing `keyword_message` unmodified plus one new `send_instagram_message` node. Deliberately "lightweight, no CRM": Instagram contacts are `IGCONTACT#{companyId}#{igsid}` records, never `LEAD#` — `CustomerIdentityService.js`/ADR-013 is untouched, no pipeline/stage/assignedTo, no `lead_created`/`tag_added`/`stage_changed` firing for Instagram. Genuinely headless v1 — no dashboard surface reads conversation history yet. Comment-to-DM/story-reply/story-mention are structurally stubbed in the webhook route (logged, 200'd) for a v2 that doesn't require a redesign. **2026-07-19 (Era 55, comment-to-DM v2):** the comments branch is now real — a `comment_received` trigger (config `{ keywords, matchMode, mediaId }`; specific post/Reel targeting required, "all posts" deferred to v3) fires on a top-level comment, sends a private-reply DM #1 (`InstagramSendService.sendPrivateReply`, `recipient: { comment_id }`), and — for the Follow Gate two-step — waits on a new `wait_instagram_reply` node and sends DM #2 once the user replies, resumed by the new IGSID-keyed `resumeOnInstagramReply` (the one genuinely new engine primitive; `resumeOnButtonReply` is phone/button-specific and the two are isolated by construction). Per-comment idempotency (`IGCOMMENT#` claim, since Meta allows exactly one private reply per comment and retries webhooks), a self-comment guard (comment-side analog of the DM `is_echo` guard), top-level-only (`parent_id` skip), and mandatory anti-spam `replyVariants` all shipped; a resuming reply is *consumed* (no double `keyword_message` fire). Still headless — configured by direct DynamoDB writes; a read-only `GET /api/instagram/media` ships as the future mediaId-picker data source. Story-reply/story-mention remain stubbed. See ADR-021. **2026-07-19 (Era 56, Instagram page v3, 4 PRs):** a consolidated, admin-only `/instagram` page ships — Messages (DM threads, forked from the WhatsApp Inbox's `ConversationList`/`ThreadPane`, not reused — igsid-keyed, text-only, no CRM) and Comments (post-grouped, reading the new `IGPOST#` store) tabs, backed by 4 new read routes plus a manual `POST /posts/:mediaId/comments/:commentId/reply` for operator-initiated private replies (409s an already-`replied` comment — Meta's one-reply-per-comment limit — else the same send path the automated Follow Gate uses), and real-time `instagram_message`/`instagram_comment` WS events. Reached from the Inbox page via a new WhatsApp/Instagram icon split next to Broadcast (byte-identical, untouched); the Instagram icon is admin/superadmin-gated to match the page's own access control, since `/inbox` itself has no role gate. Settings → Instagram is retired entirely — its connect/disconnect UI is now a single shared `InstagramSettingsPanel`, configured ONLY from the new page's own Settings tab, not duplicated. See ADR-022. *(08_MODULES.md `AutomationEngine.js`, `automations.js`, `whatsapp.js`, `handler.js`; 19_DECISION_LOG.md Era 6, Era 10, Era 19, Era 20, Era 44, Era 53, Era 54, Era 55, Era 56; docs/adr/ADR-019, ADR-020, ADR-021, ADR-022)* |
...
| **Knowledge Center / Document Knowledge / RAG retrieval** | Shipped end to end across a 3-PR arc (structured entries go semantic → documents become extractable/chunked/embedded → both are wired into the live conversational agent, entries-first-additive so documents can never outrank an admin-curated entry). Real Voyage AI (`voyage-finance-2`) embeddings throughout, one shared per-turn embed call for entries+chunks together. The original pre-launch blocker (Voyage account had no payment method, capped at 3 requests/minute) was found during Era 29 and **resolved the same day** — a payment method + $5 recharge were added; empirically re-confirmed at Era 31's closeout (5 sequential + 10 concurrent real embed calls, zero pacing, all succeeded). Chunk retrieval is an explicitly interim in-process cosine scan (ADR-018), not a vector index — a separate, still-open, lower-urgency scalability decision, not a launch blocker. The surrounding Phase 2A AI Administration/Prompt Management route layer this sits inside is not yet covered by this table — a separate, pre-existing gap. *(07_DATABASE.md §2.36; 08_MODULES.md RAG / Knowledge Center service cluster; 19_DECISION_LOG.md Era 26-31; docs/adr/ADR-017, ADR-018)* |
...
- **Meta App Review is submitted and awaiting Meta's response** (submitted 2026-07-12; Meta's
  stated review window is up to 20 days). The `apforce.in` public marketing page (a same-day
  host-based-routing addition to the existing dashboard Vercel project) and the public legal pages
  (Privacy Policy, Terms, Data Deletion — shipped 2026-07-10) were both built specifically as
  prerequisites for this submission. Tracked until resolved in `docs/PENDING_WORK.md`.
