...
| **Automations** | Shipped — a trigger/condition/step workflow engine (`AutomationEngine.js`) supporting template sends, employee assignment, stage changes, tagging, task creation, and timed waits, with a distributed-claim pattern so concurrent ticks can't double-resume a paused execution. Built after ADR-012/013 existed and correctly cites both in its own commit message and code comments. Zero automated test coverage (see §3). *(08_MODULES.md `AutomationEngine.js`; 19_DECISION_LOG.md Era 6)* |
