...
**Test coverage:** `tests/automationEngine.test.js` — the original 7 tests (linear runner: `change_stage` validation, `send_template` Meta-error surfacing) pass unchanged after the refactor; 7 new tests cover the graph runner (branching walk with live re-fetch, plain-wait pause/resume, `button_reply` pause + event-driven resume + timeout-fallback resume, dangling-edge handling).
