...
**Key exports** (singleton class instance): `fireTrigger(companyId, triggerType, context)` — entry point; `resumeExecution(companyId, waitRecord, resolvedBranch = null)` — dispatches to `_runSteps`/`_runGraph` based on `waitRecord.graph`; `processDueWaits(companyId)` — unchanged signature, still the time-based sweep for both shapes; `resumeOnButtonReply(companyId, phone10, buttonId)` — new, the event-driven reply-resume path, queries the whole `AUTO_WAIT#{companyId}` partition (a Query on a known PK, not a Scan — accepted at today's scale, same reasoning as ADR-014's `CampaignScheduler` Scan).
