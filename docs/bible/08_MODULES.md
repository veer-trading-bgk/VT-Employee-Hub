...
**Inbound webhook message handling (updated 2026-07-05):** `list_reply` (a tap on a Message+List row — the `send_list` automation node's Interactive List message) is now parsed via `isListReply()`/`parseListReply()` (mirroring `isButtonReply()`/`parseButtonReply()`, both exported for tests), stored as a first-class `MSG#` record (`type: 'list_reply'`, `listReplyId`, optional `listReplyDescription`), and WS-notified — full parity with `button_reply`. Previously this was a real gap, not just undocumented: a tapped list row was silently dropped by the message-type guard (not stored, no WS push), confirmed by direct read before the fix. Both known-lead and unknown-contact branches also now call `AutomationEngine.fireTrigger(companyId, 'keyword_message', {..., messageText: text})` inside their existing `isNewMsg` block, on every inbound `text` message or `button_reply`/`list_reply` tap — the new `keyword_message` trigger type (see this file's `AutomationEngine.js` entry above), independent of and additional to the pre-existing `resumeOnButtonReply()`/`fireButtonFollowUp()` calls in the same `if (buttonReply)` block. `list_reply` is deliberately **not** wired into `resumeOnButtonReply()`/`fireButtonFollowUp()` — those solve a different, narrower problem (resuming one specific paused graph execution / firing one welcome-message's own configured follow-up by exact button id) that this change didn't touch.
...
### `src/routes/automations.js` (updated 2026-07-05 — inbound webhook trigger)
...
**Endpoints:** `GET /stats`, `GET /executions`, `POST /_tick` (JWT-admin manual trigger path), `GET /`, `POST /`, `POST /:id/duplicate`, `GET/PUT /:id`, `PUT /:id/status`, `DELETE /:id` — all mounted under the `authMiddleware`-gated router. Plus one route that is **not** part of that router at all (see below).

**Key requires():** `../middleware/auth`, `../middleware/rateLimiter`, `../config/dynamodb`, `../services/AutomationEngine`, `../services/CustomerIdentityService` (new, aliased `CIS`), `../core/entityKeys` (new, for `leadPK()`), `crypto` (new).

**Notable exported function:** `runAutomations(companyId, triggerType, context)` — a thin wrapper around `AutomationEngine.fireTrigger()`, exported so `crm.js`, `whatsapp.js`, `campaigns.js`, and `forms.js` can fire workflow triggers without importing `AutomationEngine` directly. (The file's own comment previously named only the first three callers — `forms.js`'s two `runAutomations('lead_created', ...)` call sites, in its Meta Lead Ads webhook and its public form-submit route, existed unmentioned; fixed inline as part of the 2026-07-05 change, no separate investigation needed since both call sites were confirmed live by direct read.)

**Dual `/_tick` path — intentional, not a bug:** `src/app.js` intercepts `POST /api/automations/_tick` **before** the `authMiddleware`-gated router mount (`app.post('/api/automations/_tick', automationsRoutes.processTick)`) so the EventBridge scheduler can hit it without a JWT. The router's own `POST /_tick` (gated by `checkRole(['admin'])`) is a secondary path for manual/admin-triggered ticks.

**New: `POST /api/automations/webhook/:companyId/:workflowId/:token` (2026-07-05) — public, no auth, mounted the same way as `/_tick`.** `src/app.js` registers `app.post('/api/automations/webhook/:companyId/:workflowId/:token', automationsRoutes.inboundWebhook)` **before** the `authMiddleware`-gated router mount; `inboundWebhook` is exported as `[rateLimit(30, 60_000), handleInboundWebhook]` (an array, not a single function — lets `app.js` mount it in one line without importing `rateLimiter` separately). Capability-URL auth model: the token in the path (not a header, not a shared HMAC secret) is the actual bearer credential — generated via `crypto.randomBytes(24).toString('hex')`, compared via `crypto.timingSafeEqual` (length-checked first, since `timingSafeEqual` throws on a length mismatch rather than returning false), 404 (not 401/403) on any mismatch so a guesser can't distinguish "wrong token" from "workflow doesn't exist." Also guards Content-Length against a 100KB cap (the shared global `express.json({limit:'10mb'})` in `app.js` is far too generous for this one unauthenticated route). Flow: look up the workflow directly by `companyId`/`workflowId` (no scan) → verify `trigger.type === 'inbound_webhook'` and the token → verify the workflow is active → validate `req.body.phone` → `CIS.resolveOrCreate(companyId, {phone, name, email, source: 'inbound_webhook'}, {createdBy: 'inbound_webhook'})` (ADR-013) → one direct-key `dynamodb.get` by `leadPK(companyId, result.leadId)` for the live lead record (needed because `CIS.resolveOrCreate()`'s own contract only returns `.lead` on a fresh create — an enriched/idempotent hit returns `leadId` only) → `AutomationEngine.runWorkflowDirect(companyId, workflow, context)`. Deliberately does **not** reject duplicate submissions the way `forms.js`'s public form-submit route does (409 on an existing phone) — an external system re-notifying about the same contact is normal for a webhook integration, not a UI double-click, so the workflow fires every time a valid event arrives (same "no auto-suppression" philosophy as the `keyword_message` trigger).

**`buildTriggerForStorage(trigger, existingTrigger)`** (used by both `POST /` and `PUT /:id`) gained an `inbound_webhook` case: generates a fresh token via `crypto.randomBytes` on create; on update, preserves the existing token unless the request body sets `trigger.regenerateToken: true` (a client-only flag, never itself persisted — only `{type, conditions, webhookToken}` is written). `POST /` always generates fresh (no `existingTrigger` arg passed), since a create has nothing to preserve.
...
### `src/services/AutomationEngine.js` (updated 2026-07-04 — branching automation builder, Phase 1)

**Purpose:** Orchestrates workflow automation — fires triggers, evaluates conditions, runs actions. Two execution shapes now coexist, never mixed within one workflow: **linear** (legacy `workflow.steps[]`, run by `_runSteps()`) and **graph** (`workflow.nodes[]`/`edges[]`/`entryNodeId`, run by `_runGraph()`). `_startExecution()` dispatches on whether `workflow.nodes` is a non-empty array — a workflow is graph-shaped or linear-shaped for its whole lifetime, determined purely by which field is present, with zero migration for existing linear workflows.

**Owns:** `AUTO_EXEC#${companyId}` execution records (90-day TTL) — linear executions store results in `steps[]`, graph executions in an append-only `path[]` (one entry per node visited, since a graph only visits one path per run). `AUTO_WAIT#${companyId}` deferred-resume records (7-day grace TTL past resume time for a graph `wait` node; a `button_reply` condition node's TTL is bounded by `UNBOUNDED_REPLY_WAIT_MS`, 30 days, when it has no configured timeout). Workflow run-count/last-run-at stats.

**Graph node types:** the five original action types (`send_template`, `assign_employee`, `change_stage`, `add_tag`, `create_task`) plus `wait`/`end` — all reused unmodified via `_runAction()`, since a graph node's `{id, type, config}` shape is identical to a legacy step's — plus a new `condition` node type with three modes:
- `field_match` — evaluates `node.config.field`/`operator` against each branch's own `value`, first match wins, `fallbackKey` if none match. **Live re-fetch:** if `context.leadPK` is present, re-reads the lead's current `METADATA` before evaluating (not the frozen context captured when the trigger fired) — this is what lets a condition placed after a `wait` see e.g. "has the stage changed since we started waiting." Falls back to frozen context for leadPK-less (unknown/INBOX) contacts or if the re-fetch itself fails.
- `boolean` — same live-refetch mechanics, a single field/operator/value check, `'yes'`/`'no'` branches.
- `button_reply` — inherently a **pause point** (the node itself sends nothing; a preceding `send_template`/interactive node already did). Reuses the `AUTO_WAIT#` distributed-claim resume infra with a race between two resume paths: an inbound button tap (`whatsapp.js`'s webhook → `AutomationEngine.resumeOnButtonReply(companyId, phone10, buttonId)`, claims via the same conditional-delete pattern `processDueWaits()` uses, then resumes down the tapped button's matching branch) or the node's own timeout (the existing `processDueWaits()` time sweep, resumes down `fallbackKey` when no `resolvedBranch` is passed to `resumeExecution()`).

**Key exports** (singleton class instance): `fireTrigger(companyId, triggerType, context)` — entry point; `resumeExecution(companyId, waitRecord, resolvedBranch = null)` — dispatches to `_runSteps`/`_runGraph` based on `waitRecord.graph`; `processDueWaits(companyId)` — single-company Query sweep, kept for the JWT-admin manual `/_tick` route; `processAllDueWaits()` (new, 2026-07-06) — table-wide `Scan` across every company's `AUTO_WAIT#` partition, the variant actually wired to `handler.js`'s 5-minute EventBridge tick (see Era 19, `19_DECISION_LOG.md` — `processDueWaits()` was documented as belonging on that schedule since Era 10 but never actually was); both share a `_claimAndResume(items)` helper for the conditional-delete claim + dispatch loop. `resumeOnButtonReply(companyId, phone10, buttonId)` — the event-driven reply-resume path, queries the whole `AUTO_WAIT#{companyId}` partition (a Query on a known PK, not a Scan — accepted at today's scale, same reasoning as ADR-014's `CampaignScheduler` Scan).

**New trigger type (2026-07-05): `keyword_message`.** Unlike every other trigger type, `trigger.type === 'keyword_message'` alone doesn't decide whether a workflow fires — its own `trigger.config` (`{matchMode: 'exact'|'contains'|'any_of', keywords: string[], caseSensitive?: boolean}`) does, checked per-event via `_matchesKeywordConfig(config, context.messageText)` (line 683) inside `fireTrigger()`'s workflow filter, before the existing `trigger.conditions[]` AND-filter runs (unchanged, still stacks on top). `'contains'` and `'any_of'` share one evaluation path (substring-match against a keyword list, 1 entry vs. N) — kept as two UI options only because the config UI exposes them separately. Fails closed (never fires) on a missing/malformed config. Fired from `whatsapp.js`'s webhook on every inbound text message or button/list tap (not just first contact, unlike `whatsapp_conversation_started`) — see that file's entry below for the two call sites and the `list_reply` fix this trigger's button-tap half depended on. `src/routes/automations.js` validates `trigger.config` for this type at save time (`validateKeywordTriggerConfig()`) and — a fix in the same commit — now actually persists `trigger.config` for every trigger type; previously both `POST /` and `PUT /:id` hardcoded the stored trigger to `{type, conditions}` only, which would have silently dropped any `config` field.

**Per-button/row reply handles on `send_buttons`/`send_list` (2026-07-05):** these two node types can now pause themselves directly, the same way a `button_reply` condition node always could — but purely **opt-in**: `_runGraph()` only treats one as a pause point when it has at least one outgoing edge whose `sourceHandle` matches one of its own button/row ids (`_replyOptionIds(node)`) or the reserved `TIMEOUT_HANDLE_ID` (`'__timeout__'`). Every workflow predating this feature has only a single edge with `sourceHandle: null`, so it falls straight through to the unchanged generic Action-node branch — zero behavior change. `cta_buttons` mode never opts in (`_replyOptionIds` returns `[]` for it — a CTA/URL tap generates no webhook event, so there's nothing to ever branch on). New config fields `replyTimeoutAmount`/`replyTimeoutUnit` (flat, mirroring `ConditionNodeConfig`'s naming) control the wait, falling back to `UNBOUNDED_REPLY_WAIT_MS` when unset. `resumeExecution()` now recognizes `send_buttons`/`send_list` as a second reply-wait shape alongside `condition`+`button_reply` (`isSendReplyWait`); `resumeOnButtonReply()` resolves the branch key differently per node type — `condition` still maps a tapped button id through its own `branches[].buttonId → key` list, but for `send_buttons`/`send_list` the tapped id **is** the branch key directly (no indirection, since the button/row's own id is the handle id). Canvas rendering (`SendButtonsNode.tsx`/`SendListNode.tsx`) mirrors `ConditionNode.tsx`'s per-branch handle + "no outgoing edge" warning pattern, while keeping the original unnamed default handle so any pre-existing edge keeps rendering unchanged. The Condition editor's upstream-button-lookup convenience (`findUpstreamSendButtonsNode`/`getUpstreamButtons` in `automationGraph.ts`, and the `upstreamButtons` prop threaded through `WorkflowCanvas.tsx` → `NodeConfigPanel.tsx` → `ConditionEditor.tsx` → `BranchListEditor.tsx`) was removed as dead weight now that direct handles are the better mechanism for that case — `condition`+`button_reply` mode itself remains fully supported (`BranchListEditor.tsx` keeps its freeform button-id text input), since it's still the only option for chains that don't originate from a `send_buttons` node.

**New trigger type (2026-07-05): `inbound_webhook`.** An external system POSTs to a capability URL — `POST /api/automations/webhook/:companyId/:workflowId/:token` — naming one exact workflow, no auth beyond the URL's own token (`crypto.randomBytes(24).toString('hex')`, generated/preserved/regenerated by `buildTriggerForStorage()` in `automations.js`, compared via `crypto.timingSafeEqual`). New public method `runWorkflowDirect(companyId, workflow, context)` dispatches straight to `_startExecution()` (`triggerType: 'inbound_webhook'`) — deliberately bypassing `fireTrigger()`'s "scan every workflow of this trigger type + evaluate `trigger.conditions[]`" path, since the URL already resolved the one exact workflow to run. See `src/routes/automations.js` entry below for the route itself.

**New trigger type (2026-07-17): `flow_completed`.** Fires from `whatsapp.js`'s `nfm_reply` webhook path when an inbound WhatsApp Flow response's flowId correlation resolves — INCLUDING an ambiguous best-guess match (`flowIdConfidence: 'ambiguous'`, still enough to route on); an uncorrelated response (`flowId: null`) never fires it. The fire site sits after the correlation `try/catch` with `matchedFlowId` hoisted out, so a failed MSG#-stamp write cannot block routing while a failed resolution correctly skips it. Context = the same base shape as the `keyword_message` fire sites plus `flowId`/`flowName`/`flowFields`/`flowSummary` (`parseFlowResponse()`'s output). Uses the same per-trigger-config mechanism as `keyword_message` — `_matchesFlowCompletedConfig(config, context.flowId)`: `trigger.config.flowId` set → only that Flow's completions fire the workflow; unset/blank → company-wide any-Flow catch-all. **Opposite missing-config semantics to `_matchesKeywordConfig`, deliberately:** keyword fails closed (no keywords = broken workflow), flow_completed fails OPEN (no flowId = the documented catch-all, normalized to no stored config at all by `buildTriggerForStorage()` — no validation error). `add_tag` and `send_message` were verified to run unmodified as downstream steps (they read only `ctx.leadPK`/`ctx.phone`/`ctx.name` + their own `step.config`). Frontend: `flow_completed` in `TriggerType`/`TRIGGER_META`/`TRIGGER_OPTIONS`, with a `FlowPicker`-based config editor (`FlowCompletedConfigFields` in `WorkflowBuilder.tsx`, reusing `ButtonListEditor.tsx`'s exported picker same as `SendFlowEditor.tsx`).

**File-header self-documentation, verified accurate:** `// ADR-012: all WA sends delegated to WhatsAppSendService.` — confirmed: the `send_template` action case calls `WASendSvc.sendTemplate()` correctly, in both the linear and graph runners (same `_runAction()`). `// ADR-013: never creates customers; reads existing leads only.` — confirmed: no lead-creation code anywhere in this file, including the new live-refetch path (`_resolveConditionField()` reads `LEAD#…/METADATA` by an already-known `leadPK`, never creates); the `inbound_webhook` route itself creates leads, but via `CustomerIdentityService.resolveOrCreate()` in `automations.js`, not this file.

**Depended on by:** `src/routes/automations.js` (CRUD, now additionally accepts `nodes`/`edges`/`entryNodeId` with shallow referential-integrity validation via `validateGraphShape()`; also the sole caller of the new `runWorkflowDirect()`), `src/routes/whatsapp.js` (`resumeOnButtonReply()` call from both the known-lead and unknown-contact inbound button-tap branches, independent of the pre-existing `fireButtonFollowUp()` welcome-message mechanism).

**Internal dependencies:** unchanged — `uuid`, `../config/dynamodb`, `../config/logger`, `./WhatsAppSendService` (aliased `WASendSvc`), `./PipelineService`, lazy `require('../events/timeline')` inside `_tlWrite()`.

**Notable correctness details:** `add_tag` action's `list_append`/`ConditionExpression` race-avoidance is unchanged and shared by both runners. `wait`/`end` (and, in the graph runner, `condition` in `button_reply` mode, and an opted-in `send_buttons`/`send_list` pause) are still kept outside the per-action try/catch, for the same double-execution reason as before. `_finalizeExecution()` is a shared helper — both `_runSteps` (`fieldName: 'steps'`) and `_runGraph` (`fieldName: 'path'`) end there; the final-status formula (`completed`/`failed`/`partial_failure`) is unchanged, just parameterized. **`fieldName` is always aliased via `ExpressionAttributeNames` (`#f`, same pattern as `#st` for `status`) — fixed 2026-07-06 after `path` (a reserved DynamoDB keyword) was interpolated raw into the `UpdateExpression`, silently crashing every graph execution's finalize since the graph engine shipped; see Era 19 in `19_DECISION_LOG.md`.** `_evalOne()`'s operator switch was extracted into `_matchesOperator()` so the graph condition evaluator reuses it instead of duplicating it — trigger-time conditions (`_evalConditions`) and graph condition nodes (`_evalCondition`) share one operator implementation.

**Test coverage:** `tests/automationEngine.test.js` — the original 7 tests (linear runner: `change_stage` validation, `send_template` Meta-error surfacing) pass unchanged after the refactor; 7 new tests cover the graph runner (branching walk with live re-fetch, plain-wait pause/resume, `button_reply` pause + event-driven resume + timeout-fallback resume, dangling-edge handling); 13 tests cover `fireTrigger('keyword_message', ...)` and `_matchesKeywordConfig()` directly. 2026-07-05: 9 new tests cover `send_buttons`/`send_list` opt-in reply handles (backward-compat no-op, pause + store, `cta_buttons` never pausing, send-failure fallthrough, event-driven + timeout resume) and `runWorkflowDirect()`'s delegation to `_startExecution`. `tests/automationsWebhook.test.js` (new, 10 tests) and `tests/automationsTriggerStorage.test.js` (new, 7 tests) cover the webhook route and token generation/preservation respectively. 2026-07-06 (Era 19 incident): all 4 of this file's `dynamodb.update` mock setups now use `tests/helpers/dynamoReservedWords.js`'s `guardedUpdateMock()` instead of an unconditionally-resolving stub — it enforces DynamoDB's real reserved-keyword rule, which is what a new regression test (`_finalizeExecution()` `path`-aliasing) needed to actually fail before the fix and pass after it; a plain always-resolves mock can never catch an invalid-expression-shape bug like this one.
...
**Purpose:** Wraps `app.js` (Express) with `serverless-http` for API Gateway, and forks off a separate path for EventBridge scheduled events (campaign sweeps, lead scoring, and automation wait sweeps) that never touch Express at all.

```js
exports.handler = async (event, context) => {
  await loadSecrets();  // no-op after first cold start
  if (event.source === 'aws.events' && event['detail-type'] === 'Scheduled Event') {
    // bypasses Express/serverless-http entirely; one failing sweep never blocks the others
...
`AutomationEngine.processAllDueWaits()` (added 2026-07-06, see Era 19 in `19_DECISION_LOG.md`) rides this same 5-minute rule the same way `runDueLeadScoring()` already did — `processDueWaits()` was documented as belonging here since Era 10 (2026-07-01) but was never actually wired in, leaving no timeout/fallback safety net for paused workflows or `DelayedResponseService` timers in production until this fix.
