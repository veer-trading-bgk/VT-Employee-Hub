...
**File-header self-documentation, verified accurate:** `// ADR-012: all WA sends delegated to WhatsAppSendService.` — confirmed: the `send_template` action case calls `WASendSvc.sendTemplate()` correctly, in both the linear and graph runners (same `_runAction()`). `// ADR-013: never creates customers; reads existing leads only.` — confirmed: no lead-creation code anywhere in this file, including the new live-refetch path (`_resolveConditionField()` reads `LEAD#…/METADATA` by an already-known `leadPK`, never creates).

**Depended on by:** `src/routes/automations.js` (CRUD, now additionally accepts `nodes`/`edges`/`entryNodeId` with shallow referential-integrity validation via `validateGraphShape()`), `src/routes/whatsapp.js` (new: `resumeOnButtonReply()` call from both the known-lead and unknown-contact inbound button-tap branches, independent of the pre-existing `fireButtonFollowUp()` welcome-message mechanism).
...
**Notable correctness details:** `add_tag` action's `list_append`/`ConditionExpression` race-avoidance is unchanged and shared by both runners. `wait`/`end` (and, in the graph runner, `condition` in `button_reply` mode) are still kept outside the per-action try/catch, for the same double-execution reason as before. `_finalizeExecution()` is a new shared helper — both `_runSteps` (`fieldName: 'steps'`) and `_runGraph` (`fieldName: 'path'`) end there; the final-status formula (`completed`/`failed`/`partial_failure`) is unchanged, just parameterized. `_evalOne()`'s operator switch was extracted into `_matchesOperator()` so the new graph condition evaluator reuses it instead of duplicating it — trigger-time conditions (`_evalConditions`) and graph condition nodes (`_evalCondition`) now share one operator implementation.

**Test coverage:** `tests/automationEngine.test.js` — the original 7 tests (linear runner: `change_stage` validation, `send_template` Meta-error surfacing) pass unchanged after the refactor; 7 new tests cover the graph runner (branching walk with live re-fetch, plain-wait pause/resume, `button_reply` pause + event-driven resume + timeout-fallback resume, dangling-edge handling). 2026-07-05: 13 new tests cover `fireTrigger('keyword_message', ...)` (exact/contains/any_of modes, case-sensitivity, fails-closed on bad config, stacking with a generic condition) and `_matchesKeywordConfig()` directly.
