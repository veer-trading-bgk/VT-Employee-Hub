...
- Inbox: `POST /send`, `GET /inbox`, `GET /inbox/ping`, `GET /inbox/unknown/:phone/messages`, `POST /inbox/unknown/:phone/send`, `PUT /inbox/:leadId/resolve`, `PUT /inbox/:leadId/reopen`, `PUT /inbox/:leadId/pin`, `PUT /contact/name`, `POST /inbox/:leadId/note`, `POST /inbox/:leadId/mark-read`, `POST /inbox/unknown/:phone/mark-read`.
...
| `contacts/` | The intended Customer 360 implementation: `ActivityPanel`, header/identity pieces, tab shell, and a `tabs/` subfolder with the 7 frozen tabs (`ProfileTab`, `ConversationTab`, `TimelineTab`, `CrmTab`, `TasksTab`, `NotesTab`) — **see the Customer360Context finding below: this implementation is not what actually ships** |
| `dashboard/` | Generic `StatsCard.tsx` |
| `inbox/` | Message-composer toolbar for the WhatsApp inbox reply box |
...
| `contacts/` | List page + `[contactId]` detail page (see finding below) |
...
#### `Customer360Context.tsx` — **defined but never mounted in production**

**State owned:** `leadId`, `contact`, `stages`, `messages`, `notes`, `timeline`, `followups`, `refresh()`, `refreshFollowups()`, backed by React Query keys `['contact', leadId]`, `['crm-pipeline']`, `['crm-followups', leadId]`. **Export:** `useCustomer360()` hook, `Customer360Provider` component.

**Verified directly:** `grep -rn "Customer360Provider" dashboard/src` returns exactly 3 hits, **all inside `Customer360Context.tsx` itself** (the error message, the props interface, the function definition). No page anywhere renders `<Customer360Provider>`. Seven files call `useCustomer360()` (`components/contacts/tabs/{CrmTab,NotesTab,TasksTab,TimelineTab,ConversationTab}.tsx`, `ActivityPanel.tsx`) — every one of them would throw at runtime if actually mounted without a provider ancestor.

**What ships instead — verified directly:** `app/(v3)/contacts/[contactId]/page.tsx` (the real, live route at `/contacts/:id`) implements its own `Contact360Content` with a direct `useQuery({ queryKey: ['contact', contactId], ... })` at line 597 — confirmed by reading the file. This is **exactly** the anti-pattern the dashboard's own `CLAUDE.md` commit-level enforcement rule prohibits: *"No component fetches `['contact', leadId]` directly — all tabs consume via `useCustomer360()`."* The compliant implementation (`Customer360Provider` + the 7 tab files) exists in the codebase but is not wired up; the shipped route bypasses it entirely with its own hardcoded tab list (`overview/conversations/notes/followups/timeline/kyc/documents`, which doesn't even name-match the frozen 7: `kyc` isn't on the frozen list, and `Profile`/`CRM`/`Documents` don't line up 1:1 with what's shipped).
...
1. Two parallel, fully-built Customer 360 implementations exist. The one that matches the documented architecture (`Customer360Provider` + 7 tab components) is unmounted and dead. The one that ships (`app/(v3)/contacts/[contactId]/page.tsx`) violates the dashboard's own `CLAUDE.md` fetch-ownership rule.
