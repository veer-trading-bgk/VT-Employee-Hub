...
- **PR 5** (`ed3b857`) — `POST /api/whatsapp/send-test`: sends an approved template to any E.164 phone number for onboarding/troubleshooting verification, before any real lead exists. Reuses `WhatsAppSendService.sendTemplate()` unmodified via its existing `resolvedContact` escape hatch — stays inside ADR-012, zero changes to that shared file. Test sends live under a dedicated `TESTSEND#` namespace, confirmed live to never touch `LEAD#`/`INBOX#`. Live-verified end-to-end: this WABA had zero approved templates going in (confirmed directly against Meta); submitted a real UTILITY template for genuine review, then — once a separately-approved template became available — sent a real message to a real phone number and got back a real WAMID.

**PR 1-5: Production Complete** (backend). All five deployed via the normal `main`-push pipeline (per "Current Model" above — no separate release process exists or was introduced for this effort), each with its own passing GitHub Actions run and live production verification before commit. Remaining, not yet shipped as of this entry: a consolidated frontend panel (PR 6).
