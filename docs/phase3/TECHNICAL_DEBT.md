...
Tests (`tests/authSelfProfile.test.js`, 15 cases): `role`/`panNumber`/`aadhaarNumber`/`status`/`email`/`teamLeadId`/`baseSalary`/`autoAssignEnabled`/`autoAssignWeight` in the payload are each independently rejected with no DB write; a client-supplied `id` in the body is rejected the same way; allowed fields succeed; 404 on a since-deleted user; response never leaks `password`/`totpSecret`/`backupCodes`; avatar-upload-url rejects video/oversized/missing-params before any S3 call. Full backend suite: 98/1565 green.
