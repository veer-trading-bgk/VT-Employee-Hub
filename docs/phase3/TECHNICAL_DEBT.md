...
**Not yet done — flagged, not scoped:** a repo-wide sweep for other `.strict()`-schema consumers with the same "raw `dynamodb.get().Item` handed back as an API response, later round-tripped into a `.strict()` PUT" shape has not been performed. Today's find was `whatsapp.js` specifically; the underlying pattern is not file-specific, and a third instance elsewhere in the codebase (`ai.js`, `admin.js`, `automations.js`, etc. — not checked) would reproduce the identical bug class a third time. `src/utils/validation.js`'s header comment has been updated to note this gap inline.

**Priority:** Low (fixed) for the concrete instance; the unscoped repo-wide sweep is worth doing opportunistically rather than urgently — this bug class is annoying (a confusing 400 on a second save) but not a security or data-integrity issue, and each instance is cheap to fix once found.
