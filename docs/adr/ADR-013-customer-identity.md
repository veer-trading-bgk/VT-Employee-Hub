...
- **The loser's "re-read the winner" step is a `company-phone-index` GSI query — eventually consistent, not immediately consistent.** The `LEAD_PHONE#` lock itself (a base-table write) is immediately visible the instant the winning transaction commits, but the winner's `LEAD#` METADATA item can take a short-but-real window to become visible via the GSI. **Confirmed in production (2026-07-03, CloudWatch Logs):** a single `phoneNorm` raced 3 times within ~2 seconds (`POST /api/crm/leads`, the endpoint the Inbox's unknown-contact "assign" picker calls), and the GSI lookup found no winner on any of the 3 attempts — every one surfaced a raw, unhandled `TransactionCanceledException` ("Transaction cancelled, please refer cancellation reasons...") instead of resolving as an enrich. **Fixed:** the loser's re-read now retries with backoff (up to 4 extra attempts, 250ms/500ms/750ms/1000ms) before giving up — see `_createCustomer`'s catch block in `CustomerIdentityService.js`. This does not change the happy path (a single query, same as before) — it only engages when the phone lock has already been claimed by a concurrent create.
